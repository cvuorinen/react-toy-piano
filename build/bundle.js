...
/***/ function(module, exports, __webpack_require__) {
	    ...
	    _createClass(Settings, [{
	        ...
	        value: function render() {
	            ...
	            var selectColor = function selectColor(color) {
	                return function () {
	                    return _this.props.onSelectColor(color);
	                };
	            };
	            var colorList = this.props.colorSchemes.map(function (color) {
	                return React.createElement('div', { className: 'color-' + color, onClick: selectColor(color) });
	            });
	            return React.createElement(
	                'div',
	                { className: 'settings' },
	...
	var Score = (function (_React$Component4) {
	    ...
	    _createClass(Score, [{
	        key: 'getNoteClassName',
	        value: function getNoteClassName(noteWithTime) {
	            var note = undefined,
	                time = undefined;

	            var _noteWithTime$split = noteWithTime.split(',');

	            var _noteWithTime$split2 = _slicedToArray(_noteWithTime$split, 2);

	            note = _noteWithTime$split2[0];
	            time = _noteWithTime$split2[1];

	            var classes = ['note', note, note[0], 't' + time];
	        ...
	        value: function render() {
	            ...
	            var clef = React.createElement(
	                'note',
	                null,
	                React.createElement('span', { className: 'note clef' })
	            );
	            var scoreList = this.props.song.score.map(function (score, index) {
	                return React.createElement(
	                    'div',
	                    { className: 'score', onClick: _this2.props.onNextSong },
	                    index === 0 ? clef : null,
	                    score.map(function (note) {
	                        return React.createElement(
	                            'note',
	                            null,
	                            React.createElement('span', { className: _this2.getNoteClassName(note) })
	                        );
	                    })
	                );
	            });
	    ...
	    _createClass(Piano, [{
	        ...
	        value: function render() {
	            ...
	            return React.createElement(
	                ...
	                { className: 'piano',
	                    ref: function (node) {
	                        return node && node.setAttribute('touch-action', 'none');
	                    } },
	                keyList
	            );
	        }
	...
	var Key = React.createClass({
	    ...
	    getClassName: function getClassName() {
	        var classes = ['key', this.props.note[0], 'o' + this.props.note[1]];
